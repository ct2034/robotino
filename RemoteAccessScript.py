...
try:
    ...
    print "argument not entered!exiting script"
    exit()
...
dockercmd_getimages = "curl -v http://" + ip + ":2375/images/search?term=" + image + " | jq '.[] | .name'"
...
var = md5checksum(image,dockercmd_getimages)
if (var == False):
    createDocker(image)
