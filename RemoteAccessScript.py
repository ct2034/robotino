...
def createDocker(image, ip):
    ...
    buildcode = "curl -v -X POST -H " + '"Content-Type:application/tar"' + \
        " --data-binary '@Dockerfile.tar.gz' http://" + ip + \
        ":2375/build?t=" + image + md5.new(image).hexdigest()
...
dockercmd_runimg = "docker -H tcp://" + ip + ":2375 run -it --rm -u " + \
    '"$(id -u):$(id -g)"' + " -v $HOME/.ros:/.ros -v $HOME/.config/catkin:/.config/catkin:ro -v $(pwd):$(pwd) -w $(pwd) " + \
    image + md5.new(image).hexdigest()
