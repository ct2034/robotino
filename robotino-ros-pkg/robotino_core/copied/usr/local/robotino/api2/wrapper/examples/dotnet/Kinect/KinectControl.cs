...
namespace rec.robotino.api2.examples.kinect
{
    ...
    public partial class KinectControl : UserControl
    {
        ...
        public KinectControl(Robot robot)
        {
            this.robot = robot;
            robot.ImageReceived += new Robot.ImageReceivedEventHandler(robot_ImageReceived);
            robot.DepthReceived += new Robot.DepthReceivedEventHandler(robot_DepthReceived);
            depth = false;
            InitializeComponent();
        }
        ...
        void robot_DepthReceived(Robot sender, Image img)
        { 
            this.img = img;
            if (this.InvokeRequired)
                this.Invoke(new MethodInvoker(Invalidate));
