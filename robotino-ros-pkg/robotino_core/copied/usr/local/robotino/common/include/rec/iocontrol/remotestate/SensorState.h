...
#define REC_IOCONTROL_REMOTESTATE_SENSORSTATE_KEY "REC_IOCONTROL_REMOTESTATE_SENSORSTATE_KEY"
		...
		namespace remotestate
		{
			...
			*/
			...
			inline void SensorState::toQDSAProtocol( unsigned char* buffer ) const
			{
				...
				*(buffer+6) = ( isPowerButtonPressed ? 1 : 0 );
				*(buffer+7) = firmware_version[0];
				*(buffer+8) = firmware_version[1];
				*(buffer+9) = firmware_version[2];
			...
			inline bool SensorState::fromQDSAProtocol( const unsigned char* data )
			{
				...
				isPowerButtonPressed = ( *(data+6) > 0 ? true : false );
			...
			inline void SensorState::copy( const SensorState& other )
			{
				...
				isPowerButtonPressed = other.isPowerButtonPressed;
